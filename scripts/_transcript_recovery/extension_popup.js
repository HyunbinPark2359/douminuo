(function () {
  ...
  var calcFillAtkBtn = document.getElementById('calcFillAtkBtn');
  var calcFillDefBtn = document.getElementById('calcFillDefBtn');
  ...
  function runCalcFill(onlyAttacker, onlyDefender) {
    setCalcError('');
    setCalcResult('');
    var atkUrl = calcAtkUrlEl.value.trim();
    var defUrl = calcDefUrlEl.value.trim();

    if (onlyAttacker && !atkUrl) {
      setCalcError(mapCalcCode('empty_url'));
      return;
    }
    if (onlyDefender && !defUrl) {
      setCalcError(mapCalcCode('empty_url'));
      return;
    }

    calcSpinnerEl.hidden = false;
    calcFillAtkBtn.disabled = true;
    calcFillDefBtn.disabled = true;
    ...
    chrome.runtime.sendMessage(
      ...
      function (bg) {
        if (!bg || !bg.ok) {
          ...
          calcFillAtkBtn.disabled = false;
          calcFillDefBtn.disabled = false;
          setCalcError((bg && bg.error) || '페이로드를 만들지 못했습니다.');
          return;
        }

        var pl = bg.payloads || {};
        if (onlyAttacker && pl.attacker && pl.attacker.error) {
          calcSpinnerEl.hidden = true;
          calcFillAtkBtn.disabled = false;
          calcFillDefBtn.disabled = false;
          setCalcError(mapCalcCode(pl.attacker.error));
          return;
        }
        if (onlyDefender && pl.defender && pl.defender.error) {
          calcSpinnerEl.hidden = true;
          calcFillAtkBtn.disabled = false;
          calcFillDefBtn.disabled = false;
          setCalcError(mapCalcCode(pl.defender.error));
          return;
        }
  ...
  calcFillAtkBtn.addEventListener('click', function () {
    runCalcFill(true, false);
  });
  calcFillDefBtn.addEventListener('click', function () {
    runCalcFill(false, true);
  });
