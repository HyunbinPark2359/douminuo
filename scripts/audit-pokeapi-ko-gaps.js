...
const ABILITY_FB = path.join(ROOT, 'extension', 'abilityKoFallback.json');
...
function annotateFallbacks(moveResult, itemResult, abilityResult) {
  ...
  const abilitiesStillNeedingKo = abilityResult.noKoreanName.filter(
    (a) => !abilityFbValues.has(a.pokeapiSlug)
  );
  const abilitiesCoveredByFallback = abilityResult.noKoreanName.filter((a) =>
    abilityFbValues.has(a.pokeapiSlug)
  );
  ...
  return {
    ...
    abilitiesCoveredByAbilityKoFallback: abilitiesCoveredByFallback.length,
    abilitiesStillNeedingKoreanLabel: abilitiesStillNeedingKo,
  };
}
...
async function main() {
  ...
  const extra = annotateFallbacks(moveResult, itemResult, abilityResult);
  ...
  const report = {
    ...
    abilitiesFallbackSummary: {
      coveredByExtensionAbilityKoFallback: extra.abilitiesCoveredByAbilityKoFallback,
      stillNeedKoreanStringCount: extra.abilitiesStillNeedingKoreanLabel.length,
      stillNeedList: extra.abilitiesStillNeedingKoreanLabel,
    },
  };
    ...
    JSON.stringify(
      {
        ...
        abilitiesStillAfterFallback: extra.abilitiesStillNeedingKoreanLabel.length,
