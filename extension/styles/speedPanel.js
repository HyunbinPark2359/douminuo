...
(function (g) {
  ...
  g.nuoSpeedPanelCss = [
    ...
    '}',
    /* 펼치기 버튼 + 구분선 */
    '.expand-sep { height: 1px; background: #d9d9d9; margin: 10px 0 8px; }',
    '.expand-btn {',
    '  display: block; width: 100%; background: transparent; border: 0; cursor: pointer;',
    '  font-family: SCDream, sans-serif; font-size: 10px; color: #636363;',
    '  letter-spacing: -0.3px; padding: 8px 0 2px; text-align: center;',
    '  transition: color .2s cubic-bezier(.25,.8,.5,1);',
    '}',
    '.expand-btn:hover { color: #e4007f; }',
    /* 인라인 펼침 영역 — max-height 로 아래 확장 */
    '.inline-table-wrap { max-height: 0; overflow: hidden; transition: max-height .3s cubic-bezier(.25,.8,.5,1); }',
    '@media (prefers-reduced-motion: reduce) { .inline-table-wrap { transition: none; } }',
    ...
    /* 커스텀 오버레이 스크롤바 (네이티브 숨김 → fade) */
    '.inline-sb { position: absolute; top: 0; bottom: 0; right: 1px; width: 6px; opacity: 0; transition: opacity .35s ease; pointer-events: none; z-index: 5; }',
    '.inline-sb.show { opacity: 1; pointer-events: auto; }',
    '.inline-sb-thumb { position: absolute; left: 1px; right: 1px; border-radius: 3px; background: rgba(148, 163, 184, 0.55); cursor: pointer; }',
    '.inline-sb-thumb:hover { background: rgba(148, 163, 184, 0.8); }',
    /* 펼친 표 상단 고정 제목 (스크롤해도 유지) */
    '.inline-table-title {',
    '  position: sticky; top: 0; z-index: 3; background: #fff;',
    '  font-weight: 700; font-size: 9px; color: #636363; line-height: 1.35;',
    '  letter-spacing: -0.2px; margin: 0; padding: 0 4px 6px 0;',
    '}',
    /* 펼친 표 divider = sticky 섹션 헤더 (제목 아래 현재 섹션 1개 고정) */
    '.inline-table-body .species-pop-divider {',
    '  position: sticky; top: var(--inline-title-h, 22px); z-index: 2;',
    '  background: #fff; margin: 0; padding: 6px 0;',
    '}',
  ].join('\n');
