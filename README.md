...
[도우미누오 크롬 웹 스토어 페이지](https://chromewebstore.google.com/detail/bmgohocnliecjcefhnaakbdcfjcdmoan?utm_source=item-share-cb)

### GitHub 릴리즈에서 설치 (웹 스토어 외)

1. GitHub 저장소의 **Releases**에서 원하는 버전을 고릅니다.
2. 해당 버전에 첨부된 **douminuo-x.x.x.zip**을 내려받고 **압축을 풉니다.**
3. 크롬에서 **확장 프로그램** 창을 엽니다.
    - **방법 1:** 주소창에 `chrome://extensions` 를 입력합니다.
    - **방법 2:** 오른쪽 위 **점 세 개(더보기)** → **확장 프로그램** → **확장 프로그램 관리** 를 클릭합니다.
4. 오른쪽 위 **개발자 모드**를 켭니다.
5. 왼쪽 위 **압축해제된 확장 프로그램 로드**를 누릅니다.
6. 위 3번 압축을 푼 폴더를 선택합니다.
7. **도우미누오** 아이콘이 보이면 설치 완료입니다.

업데이트할 때는 새 릴리즈 zip으로 같은 절차를 반복하거나, 압축을 풀어 기존 폴더를 덮어쓴 뒤 `chrome://extensions` 에서 해당 확장 프로그램의 **새로고침(↻)** 을 누르면 됩니다.
...
2. 페이지 우측 하단 플로팅 버튼에 커서를 이동합니다.
3. **토글 버튼**을 클릭하여 **공격측**과 **수비측** 중 원하는 쪽으로 전환합니다.
4. 계산기에 입력하고 싶은 **포켓몬**의 **슬롯 버튼**을 **클릭**합니다.
  4-1. 혹은 미리 저장해둔 다른 샘플을 넣고싶다면 좌측 입력란에 **단일 샘플 URL**을 넣고 **입력** 버튼을 누릅니다.
5. 확장 프로그램이 샘플을 읽고 계산기에 포켓몬 이름, 노력치, 기술, 도구, 특성 등을 자동으로 채워줍니다.
...
팝업 테마, 플로팅 버튼, 팀빌더 인라인 수치, 간단 스피드 계산기 등을 설정할 수 있습니다.

### 간단 스피드 계산기
<img src="assets/store/04-speed-calc.png" alt="간단 스피드 계산기 이미지" style="max-width: 100%; height: auto;" />

팀빌더 우측 패널에서 포켓몬의 추월 범위를 확인할 수 있습니다.
스피드 표를 통해 어느 포켓몬까지 추월하는지 쉽게 확인할 수 있습니다.
